...
async function performSwapNoFees() {
  console.log("🚀 Starting Jupiter Swap WITHOUT Fees (Testing)");
  ...
  console.log("🚫 Platform Fee: DISABLED for testing");
  console.log("");
  ...
  try {
    ...
    // Get quote WITHOUT platform fees
    console.log("📊 Getting quote (no fees)...");
    const quoteUrl =
    ...
    console.log("  Platform Fee: NONE");
    console.log("");

    // Get swap transaction WITHOUT fees
    console.log("🔄 Getting swap transaction (no fees)...");
    const swapBody = {
      quoteResponse: quote,
      userPublicKey: wallet.publicKey.toBase58(),
      // NO feeAccount parameter
    ...
    console.log("🎯 Next step: Add fee collection once basic swap works");
  } catch (error) {
...
if (require.main === module) {
  performSwapNoFees().catch(console.error);
}
